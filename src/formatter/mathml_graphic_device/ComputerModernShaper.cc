...
Char8
ComputerModernShaper::toTTFGlyphIndex(FontNameId name, Char8 index)
{
  GlyphMap* map = 0;
  switch (name)
    {
    ...
    case FN_CMSSBX:
    case FN_CMTT:
      assert(index < sizeof(cmrTTFMap) / sizeof(GlyphMap));
      map = cmrTTFMap;
      break;
    case FN_CMMI:
    case FN_CMMIB:
      assert(index < sizeof(cmmTTFMap) / sizeof(GlyphMap));
      map = cmmTTFMap;
      break;
    case FN_CMSY:
    case FN_CMBSY:
      assert(index < sizeof(cmsTTFMap) / sizeof(GlyphMap));
      map = cmsTTFMap;
      break;
    case FN_CMEX:
      assert(index < sizeof(cmexTTFMap) / sizeof(GlyphMap));
      map = cmexTTFMap;
      break;
    default:
      assert(false);
    }
  assert(map[index].source == index);
